...
виконання — рестарт dev-сервера / деплой убиває перевірку (див. альтернативу
з чергою нижче).

### Альтернатива / майбутня еволюція: черга джобів (pg-boss)

`after()` виконує сесію у фоні, але в тому самому процесі Next.js. Черга виносить
виконання за межі життєвого циклу HTTP-процесу.

**Що дає понад `after()`:**
- довговічність: джоб переживає рестарт процесу; у зв'язці з checkpoint —
  продовження з місця обриву без ручного Resume;
- retry з бекофом і контроль конкурентності (одна сесія на скрапер одночасно,
  паралель між різними скраперами);
- вбудований cron-планувальник — закриває майбутній автозапуск перевірок
  (webhook + `AutoCheckConfig` з «Майбутніх розширень» CLAUDE.md).

**Як працює (pg-boss — черга поверх нашого ж Postgres, без нової інфраструктури):**

```typescript
// app/api/checks/route.ts — замість after():
const sessionId = await createCheckSession(input)
await boss.send('check-session', { sessionId, input })
return NextResponse.json({ sessionId })

// worker: той самий Next-процес (instrumentation.ts) або окремий `npm run worker`
boss.work('check-session', async ([job]) => {
  await executeCheckSession(job.data.sessionId, job.data.input)
})
```

Процес-виконавець упав → pg-boss за TTL повертає джоб у чергу → воркер після
рестарту підхоплює і продовжує з checkpoint.

**Вартість:** один npm-пакет + службові таблиці в схемі `pgboss` у quality_db;
~150–300 рядків (ініціалізація, воркер, enqueue); операційно — нічого нового,
якщо воркер усередині Next, або один додатковий процес. Прихована ціна:
подвійний життєвий цикл (graceful shutdown × 2), дебаг менш прямолінійний.

**Рішення:** зараз — overkill (внутрішній інструмент, ручні запуски);
стартуємо з `after()` + checkpoint/resume. Черга стає виправданою в момент
реалізації автозапуску за розкладом/webhook. Ключова підготовка вже в цьому
покращенні: `executeCheckSession(sessionId, input)` відділений від
HTTP-хендлера — міграція на pg-boss потім зводиться до заміни `after(...)`
на `boss.send(...)` + файл воркера, без переписування логіки перевірок.
BullMQ+Redis розглянуто і відхилено: тягне Redis у docker-compose заради
можливостей, які нам не потрібні.
