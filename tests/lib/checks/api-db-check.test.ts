...
function makePolygons(ids: string[]) {
  return ids.map((pid) => ({ polygonId: pid, boundBox: null, polygonType: null, city: null }))
}
...
describe('runApiDbCheck', () => {
  it('counts found and not-found entities correctly', async () => {
    ...
    const result = await runApiDbCheck(baseInput, adapter, 'dockless', makePolygons(baseInput.polygonIds))
  ...
  it('deduplicates entities across multiple polygons', async () => {
    ...
    const result = await runApiDbCheck(input, adapter, 'dockless', makePolygons(input.polygonIds))
